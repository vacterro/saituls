...
def main():
    ...
    files = sorted(SCRIPT_DIR.glob("*.dds"))

    if not files:
        print("[INFO] No DDS files found")
        return

    ok = 0
    fail = 0

    print(f"[INFO] Found {len(files)} DDS file(s)\n")

    for src in files:

        dst = src.with_suffix(".tga")
